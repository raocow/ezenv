# rigor: make bare `python`/`pip` resolve to python3/pip3.
#
...
#   source "$(brew --prefix)/share/rigor/pyf.zsh"

_rigor_shim_dir="${RIGOR_SHIM_DIR:-${DEVRIG_SHIM_DIR:-${EZENV_SHIM_DIR:-$HOME/.local/share/rigor/shims}}}"
mkdir -p "$_rigor_shim_dir"

if command -v python3 >/dev/null 2>&1; then
  ln -sf "$(command -v python3)" "$_rigor_shim_dir/python"
fi
if command -v pip3 >/dev/null 2>&1; then
  ln -sf "$(command -v pip3)" "$_rigor_shim_dir/pip"
fi

# Append (not prepend) so real interpreters and venvs take precedence.
case ":$PATH:" in
  *":$_rigor_shim_dir:"*) ;;
  *) export PATH="$PATH:$_rigor_shim_dir" ;;
esac

unset _rigor_shim_dir
